...
void print_stackframe(void) {
    ...
    可以从以上链接的图看如何通过ebp取得eip(return addr)，arg1~arg4，和下一个ebp的。
    ...
    3  var arg3
    4  var arg4
    5  callee ebp  指向 caller ebp
    6  callee return addr (next) | caller eip -> 上一个指令应该为call callee
    7  callee arg4  callee 参数压栈（传参用
    8  callee arg3  callee 参数压栈（传参用
    9  callee arg2  callee 参数压栈（传参用
    10 callee arg1  callee 参数压栈（传参用
       ...
    11  var arg1
    12  var arg2
    13  var arg3
    ...
    high
    */
    uint32_t ebp = read_ebp(), eip = read_eip(); // 5, eip寄存器
    int i, j;
    for (i = 0; ebp != 0 && i < STACKFRAME_DEPTH; i++) {
        ...
        // 7 8 9 10
        for (j = 2; j < 6; j++) {
            cprintf("0x%08x ", ebp_p[j]);
        }
        cprintf("\n");
        print_debuginfo(eip - 1); // eip 指向接下来将要执行的指令 eip-1指向当前指令
        eip = ebp_p[1]; // 6 -> caller eip address
        ebp = ebp_p[0]; // 5 -> 15
    }
}
